...
class App extends Component {
  ...
  async componentDidMount() {
    ...
    // check in localStorage if we have any existing information about the customer
  ...
  _loadConversations = async customerId => {
    ...
    // if there is only one conversation and in that conversation there aren't any messages
    // we can then be sure that this is the initial creation of the user and we can get
    // started with their support questions right away - therefore open empty conversation
  ...
  _initiateNewConversation = () => {
    // get username and customer id from local storage
    const customerId = localStorage.getItem(FREECOM_CUSTOMER_ID_KEY)
    const username = localStorage.getItem(FREECOM_CUSTOMER_NAME_KEY)
    // check if current component state already has an empty conversation
    // this is here for a check so that if a user presses the "new conversation" button
    // multiple times that we do not go and create multiple conversations in the backend
